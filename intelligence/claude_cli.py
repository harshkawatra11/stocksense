...
def intraday_signal_check(signals: list[dict], learnings_context: str = "") -> list[dict]:
    ...
    signals: list of dicts with ticker, signal, confidence, ml_reasoning, slm_reasoning
...
"""
    ...
    prompt = f"""You are reviewing intraday NSE stock signals generated by ML + SLM models.
...
def eod_review_call(predictions: list[dict]) -> dict:
    ...
    predictions: [{ticker, signal, predicted_close, actual_close, ml_reasoning, slm_reasoning, claude_reasoning}]
    """
    if not predictions:
...
{{
  ...
  "learnings": [
    {{
      ...
      "model_at_fault": "ml|slm|claude|all"
    }}
  ],
  "accuracy_today": {{
    "total": int,
