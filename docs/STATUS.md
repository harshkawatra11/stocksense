...
Per `phase0_verdict.md` Run 4, the one configuration to have passed the pre-registered gate is h=10/top_n=10 (+0.447% net alpha/rebalance, 43 folds, p=0.000085) — trained, so far, only on the 98-symbol `PHASE0_UNIVERSE` of currently-liquid large caps, not the point-in-time mid/small-cap universe `universe_pit.py` already makes available (1,903 tradeable names today, 733 as of 2015, built from the 8.17M-row `bhavcopy_eq` spine). `predictions` currently holds 0 rows — every number to date is a backtest, with no forward track record. Phase G re-runs the gate on the real universe, starts the forward ledger, and adds expected-move/confidence output and a low-turnover daily brief — see the plan for detail. **Design invariant carried through every step:** no capital figure is a system parameter. The model, gate, and ledger operate in relative returns and weights only; where Indian fixed-fee costs make ticket size genuinely relevant, the system derives and publishes a break-even ticket size rather than assuming an account size.
