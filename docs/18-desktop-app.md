...
| Path | Role |
|---|---|
| `src/stocksense/server/app.py` | Read-only JSON API: `/api/health`, `/api/summary`, `/api/doshas`, `/api/counterfactuals`, `/api/positions`, `/api/harness`, `/api/registry`, `/api/agent-runs`. Every endpoint reads what a CLI command already computed and wrote — it never triggers a computation, a trade, or an agent call. |
